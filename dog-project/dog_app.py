...
# 
# - `human_files` 的前100张图像中，能够检测到**人脸**的图像占比多少？ 1
# - `dog_files` 的前100张图像中，能够检测到**人脸**的图像占比多少？ 0.11
# 
...
human_prob = len([img_path for img_path in human_files_short if face_detector(img_path) is True]) / 100
dog_prob = len([img_path for img_path in dog_files_short if face_detector(img_path) is True]) / 100
...
# 
# __回答:__
# 要求用户提供清晰面部特征的人脸图像并不合理。因为，用户可能受限于自身设备性能因素、环境因素，不能提供特别清晰的图像也是很正常的。
# 通过比较人脸轮廓与待检测图像的相似度，应该也可以检测到人脸。
# 
# 
...
# - `dog_files_short`中图像检测到狗狗的百分比？ 0

# In[11]:

### TODO: 测试dog_detector函数在human_files_short和dog_files_short的表现
human_prob = len([img_path for img_path in human_files_short if dog_detector(img_path) is True]) / 100
dog_prob = len([img_path for img_path in dog_files_short if dog_detector(img_path) is True]) / 100
...
# 
# __回答:__ 
# 开始添加三个卷积层，每个卷积层的深度逐渐增多，这样是为了消除图像空间因素带来的影响，同时也为了提取更高阶的特征。
# 其次，在每个卷积层后面都增加了一层最大池化层，这样是为了保证模型能够提取代表性特征的同时，减少输出大小，避免模型过拟合。
# 同时模型最后添加了一层全局平均池化层，作用是避免最后一层全连接层参数过多，造成过拟合。
# 模型最后一层输出层，用于输出结果。
...
### TODO: 定义你的网络架构
model.add(Conv2D(filters=16, kernel_size=(2,2), strides=1, padding='valid', input_shape=(224,224,3)))
model.add(MaxPooling2D(pool_size=(2,2), strides=2, padding='valid'))   
model.add(Conv2D(filters=32, kernel_size=(2,2), strides=1, padding='valid'))
model.add(MaxPooling2D(pool_size=(2,2), strides=2, padding='valid'))
model.add(Conv2D(filters=64, kernel_size=(2,2), strides=1, padding='valid'))  
model.add(MaxPooling2D(pool_size=(2,2), strides=2, padding='valid'))
...
# 
# __回答:__ 
# * 模型第一步是添加一个全局平均池化层，由于是第一层，需要指定input_shape参数。该参数为Resnet50最后一层卷积层的输出。
# * 模型第二步是添加一个全连接层，其中全连接层使用 softmax 激活函数，对每一个狗的种类都包含一个节点。
# * 模型使用resnet50网络架构，层数更多，模型拟合能力更强，更高阶的特征可以提取出来。这是由于resnet的特性，即由于residual block的存在，使得模型深度更深，学习能力更强，避免了plain network深度越深导致模型难以优化、效果欠佳的情况。
# * 由于数据与imageNet数据较为相似，在数据相似的前提下，迁移学习模型提取出的抽象特征理论上具有一定的通用性。
...
def detector(img_path):
    if face_detector(img_path):
        ...
        if dog_detector(img_path):
            ...
            text = "oh, sorry. I don't know what you are."
...
# 同时请回答如下问题：
# 
# 1. 输出结果比你预想的要好吗 :) ？或者更糟 :( ？ 结果与预期结果基本一致
# 2. 提出至少三点改进你的模型的想法。
# 2.1 使用数据增强方法，添加更多图像数据，避免过拟合。
# 2.2 使用正则方法，避免数据过拟合。
# 2.3 减少epoch迭代次数，避免过拟合

# In[26]:

## TODO: 在你的电脑上，在步骤6中，至少在6张图片上运行你的算法。
## 自由地使用所需的代码单元数吧
imgs = ['./images/{}.png'.format(i) for i in range(1,7)]
for path in imgs:
    text = detector(path)
    img = cv2.imread(path)
    cv_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
